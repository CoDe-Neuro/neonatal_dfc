...
kop_b <- dfm %>% 
  dplyr::group_by(sub, 
                  ...
                  language_comp, 
                  qchat_total) %>% 
  dplyr::summarise(meanSync = mean(ensembleSync, na.rm = TRUE)) %>% 
  dplyr::ungroup() %>%   
  dplyr::left_join(imd_tab) %>% 
  dplyr::mutate(age = age_m + (1/30*age_d), 
                cor_age = cor_age_m + (1/30*cor_age_d))
...
sync_res <- lm(data = kop_b, formula = meanSync ~ ga + pma + sex + motparam_fd_outlier_count + cor_age + imd)
qchat_res <- lm(data = kop_b, formula = qchat_total ~ ga + pma + sex + motparam_fd_outlier_count + cor_age + imd)
...
summary(glm(data = kop_b_filt_term, formula = meanSync ~ qchat_total + ga + pma + sex + motparam_fd_outlier_count + cor_age + imd))
summary(glm(data = kop_b_filt_pterm, formula = meanSync ~ qchat_total + ga + pma + sex + motparam_fd_outlier_count + cor_age + imd))
summary(glm(data = kop_b, formula = meanSync ~ qchat_total + ga + pma + sex + motparam_fd_outlier_count + cor_age + imd))
...
kop_mean %>% 
  dplyr::group_by(pt) %>% 
  dplyr::summarise(meanSync_m = mean(meanSync, na.rm = TRUE), 
                   meanSync_sd = sd(meanSync, na.rm = TRUE), 
                   metastab_m = mean(ensembleMetastab, na.rm = TRUE), 
                   metastab_sd = sd(ensembleMetastab, na.rm = TRUE))
```


```{r kopfig, eval=FALSE, include=FALSE}
#| echo=FALSE,
#| fig.align='center',
#| fig.height=10,
#| fig.width=14,
...
```{r}
allTransitionsPT %>% 
  dplyr::filter(!is.na(cor_p)) %>% print(n=25)
```
...
Mean KOP
State 4
*t =* `r format(round(models_term_$filt.t[models_term_$var=='Mean KOP' & models_term_$cluster==4 & models_term_$term=='PMA*'], digits = 1), nsmall = 1)` (`r models_term_$pp.for[models_term_$var=='Mean KOP' & models_term_$cluster==4 & models_term_$term=='PMA*']`)
State 5
*t =* `r format(round(models_term_$filt.t[models_term_$var=='Mean KOP' & models_term_$cluster==5 & models_term_$term=='PMA*'], digits = 1), nsmall = 1)` (`r models_term_$pp.for[models_term_$var=='Mean KOP' & models_term_$cluster==5 & models_term_$term=='PMA*']`)

FO
State 5
*t =* `r format(round(models_term_$filt.t[models_term_$var=='Fractional Occupancy' & models_term_$cluster==5 & models_term_$term=='PMA*'], digits = 1), nsmall = 1)` (`r models_term_$pp.for[models_term_$var=='Fractional Occupancy' & models_term_$cluster==5 & models_term_$term=='PMA*']`)

DT
State 2
*t =* `r format(round(models_term_$filt.t[models_term_$var=='Dwelling times' & models_term_$cluster==2 & models_term_$term=='PMA*'], digits = 1), nsmall = 1)` (`r models_term_$pp.for[models_term_$var=='Dwelling times' & models_term_$cluster==2 & models_term_$term=='PMA*']`)
State 3
*t =* `r format(round(models_term_$filt.t[models_term_$var=='Dwelling times' & models_term_$cluster==3 & models_term_$term=='PMA*'], digits = 1), nsmall = 1)` (`r models_term_$pp.for[models_term_$var=='Dwelling times' & models_term_$cluster==3 & models_term_$term=='PMA*']`)


PND
Mean KOP
State 2
*t =* `r format(round(models_term_$filt.t[models_term_$var=='Mean KOP' & models_term_$cluster==2 & models_term_$term=='PND*'], digits = 1), nsmall = 1)` (`r models_term_$pp.for[models_term_$var=='Mean KOP' & models_term_$cluster==2 & models_term_$term=='PND*']`)

FO
State 2
*t =* `r format(round(models_term_$filt.t[models_term_$var=='Fractional Occupancy' & models_term_$cluster==2 & models_term_$term=='PND*'], digits = 1), nsmall = 1)` (`r models_term_$pp.for[models_term_$var=='Fractional Occupancy' & models_term_$cluster==2 & models_term_$term=='PND*']`)

DT
State 2
*t =* `r format(round(models_term_$filt.t[models_term_$var=='Dwelling times' & models_term_$cluster==2 & models_term_$term=='PND*'], digits = 1), nsmall = 1)` (`r models_term_$pp.for[models_term_$var=='Dwelling times' & models_term_$cluster==2 & models_term_$term=='PND*']`)
State 5
*t =* `r format(round(models_term_$filt.t[models_term_$var=='Dwelling times' & models_term_$cluster==5 & models_term_$term=='PND*'], digits = 1), nsmall = 1)` (`r models_term_$pp.for[models_term_$var=='Dwelling times' & models_term_$cluster==5 & models_term_$term=='PND*']`)
...
Mean KOP
State 1
*t =* `r format(round(models_$filt.t[models_$var=='Mean KOP' & models_$cluster==1 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Mean KOP' & models_$cluster==1 & models_$term=='Born Preterm**']`)
State 2
*t =* `r format(round(models_$filt.t[models_$var=='Mean KOP' & models_$cluster==2 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Mean KOP' & models_$cluster==2 & models_$term=='Born Preterm**']`)
State 3
*t =* `r format(round(models_$filt.t[models_$var=='Mean KOP' & models_$cluster==3 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Mean KOP' & models_$cluster==3 & models_$term=='Born Preterm**']`)
State 4
*t =* `r format(round(models_$filt.t[models_$var=='Mean KOP' & models_$cluster==4 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Mean KOP' & models_$cluster==4 & models_$term=='Born Preterm**']`)
State 5
*t =* `r format(round(models_$filt.t[models_$var=='Mean KOP' & models_$cluster==5 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Mean KOP' & models_$cluster==5 & models_$term=='Born Preterm**']`)
State 6
*t =* `r format(round(models_$filt.t[models_$var=='Mean KOP' & models_$cluster==6 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Mean KOP' & models_$cluster==6 & models_$term=='Born Preterm**']`)

FO
State 1
*t =* `r format(round(models_$filt.t[models_$var=='Fractional Occupancy' & models_$cluster==1 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Fractional Occupancy' & models_$cluster==1 & models_$term=='Born Preterm**']`)
State 2
*t =* `r format(round(models_$filt.t[models_$var=='Fractional Occupancy' & models_$cluster==2 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Fractional Occupancy' & models_$cluster==2 & models_$term=='Born Preterm**']`)
State 4
*t =* `r format(round(models_$filt.t[models_$var=='Fractional Occupancy' & models_$cluster==4 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Fractional Occupancy' & models_$cluster==4 & models_$term=='Born Preterm**']`)
State 6
*t =* `r format(round(models_$filt.t[models_$var=='Fractional Occupancy' & models_$cluster==6 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Fractional Occupancy' & models_$cluster==6 & models_$term=='Born Preterm**']`)

DT
State 1
*t =* `r format(round(models_$filt.t[models_$var=='Dwelling times' & models_$cluster==1 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Dwelling times' & models_$cluster==1 & models_$term=='Born Preterm**']`)

Metastability
State 1
*t =* `r format(round(models_$filt.t[models_$var=='Metastability' & models_$cluster==1 & models_$term=='Born Preterm**'], digits = 1), nsmall = 1)` (`r models_$pp.for[models_$var=='Metastability' & models_$cluster==1 & models_$term=='Born Preterm**']`)
...
```{r clusNum, echo=FALSE, fig.height=3, fig.width=6, message=FALSE, warning=FALSE, dev='pdf'}
```
