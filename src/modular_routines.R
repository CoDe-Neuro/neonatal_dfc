...
dfm %>% 
  dplyr::select(pc1, 
                ...
                pc3, 
                cluster) %>% 
  dplyr::sample_n(10*2280) %>% 
  dplyr::mutate(cluster = as.factor(cluster)) %>% 
  dplyr::mutate(cluster = recode(cluster,
                               `0` = "Cluster #0",
                               `1` = "Cluster #1", 
                               `2` = "Cluster #2",
                               `3` = "Cluster #3", 
  ...
  plotly::plot_ly(x = ~pc1, 
                  ...
                  z = ~pc3, 
                  color = ~cluster, 
                  colors = ~mypal[c(2,4,5,6,8,1,3,7)]) %>% 
  layout(
...
fo <- dfm %>% 
  ...
  dplyr::select(ttime, sub, pma, pt, sex, motparam_fd_outlier_count, cluster) %>%
  stateR::nest_fo(., 
          vars = c('sub', 
                   'pma', 
                   'pt', 
          ...
          foVar = 'cluster') %>% 
  tidyr::unnest(c(data)) %>% 
  select(-c(pma, pt, sex, motparam_fd_outlier_count)) %>% 
  dplyr::group_by(cluster) %>% 
  dplyr::summarise(pat = list(tibble(sub,perc))) %>% 
  dplyr::ungroup() %>% 
  tidyr::spread(key = cluster, 
                value = pat) %>% 
  tidyr::unnest() %>% 
  dplyr::select(-c(sub1,sub2,sub3,sub4,sub5,sub6,sub7)) %>% 
  dplyr::select(-sub) %>% 
  tibble::as_tibble()

imglist <- c("figs/centr_k8_1_plot.png", 
             "figs/centr_k8_2_plot.png", 
             "figs/centr_k8_3_plot.png", 
             "figs/centr_k8_4_plot.png", 
             "figs/centr_k8_5_plot.png", 
             "figs/centr_k8_6_plot.png", 
             "figs/centr_k8_7_plot.png", 
             "figs/centr_k8_8_plot.png")


fox <- fo %>% 
  tidyr::gather(., key = clus, value = p) %>% 
  dplyr::group_by(clus) %>% 
  dplyr::summarise(Median = median(p), 
                   IQR = IQR(p), 
                   Min = min(p), 
                   Max = max(p), 
                   val = list(p))

fo_tab <- tibble::tibble('State' = c('0','1','2','3','4','5','6','7'), 
                         'Anat' = '', 
                         fox, 
                         'Histogram' = '') %>% 
  dplyr::select(-c(clus, val))

fox <- fox %>% 
  unnest()

fo_list <- split(fox$val, fox$clus)

fo_tab %>%   
  
  dplyr::mutate(
    Median = format(round(Median, 
                          digits = 3), 
                    nsmall = 3), 
    IQR = format(round(IQR, 
                       digits = 3), 
                 nsmall = 3),
    Min = format(round(Min, 
                       digits = 3), 
                 nsmall = 3), 
    Max = format(round(Max, 
                       digits = 3), 
                 nsmall = 3), 
  ) %>% 
  ...
  column_spec(2, image = spec_image(imglist, 
                                    width = 69.2*2, 
                                    height = 50.3*2)) %>% 
  column_spec(7, image = spec_hist(fo_list, 
                                   col = "#4eb3d3", 
                                   border = "#4eb3d3", 
                                   breaks = 20)) %>%
...
## ---- foTableTermCA_8 --------
mot_fd <- pw_ttests_term %>% 
  dplyr::select(-c(p.value, data, estimate)) %>% 
  dplyr::rename('p.value' = 'p.perm') %>% 
  dplyr::filter(term == 'motparam_fd_outlier_count') %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(p.value = p.adjust(p.value, 
                                   method = 'fdr', 
                                   n = length(p.value))) %>%
  dplyr::rename('mot_fd.stat' = 'statistic', 
                'mot_fd.p' = 'p.value') %>% 
  select(-term)
...
ca <- pw_ttests_term %>% 
  dplyr::select(-c(p.value, data, estimate)) %>% 
  dplyr::rename('p.value' = 'p.perm') %>% 
  dplyr::filter(term == 'ca') %>% 
  dplyr::ungroup() %>% 
  dplyr::mutate(p.value = p.adjust(p.value, 
                                   method = 'fdr', 
                                   n = length(p.value))) %>% 
  dplyr::rename('ca.stat' = 'statistic', 
                'ca.p' = 'p.value') %>% 
  select(-term)
...
pma <- pw_ttests_term %>% 
  ...
  dplyr::mutate(p.value = p.adjust(p.value, 
                                   method = 'fdr', 
                                   n = length(p.value))) %>% 
  dplyr::rename('pma.stat' = 'statistic', 
                'pma.p' = 'p.value') %>% 
  select(-term)
...
sex <- pw_ttests_term %>% 
  ...
  dplyr::mutate(p.value = p.adjust(p.value, 
                                   method = 'fdr', 
                                   n = length(p.value))) %>% 
  dplyr::rename('sex.stat' = 'statistic', 
                'sex.p' = 'p.value') %>% 
  select(-term)

intcpt <- pw_ttests_term %>% 
  dplyr::select(-c(p.value, data, estimate)) %>% 
  dplyr::rename('p.value' = 'p.perm') %>% 
  ...
  dplyr::mutate(p.value = p.adjust(p.value, 
                                   method = 'fdr', 
                                   n = length(p.value))) %>%
  dplyr::rename('intcpt.stat' = 'statistic', 
                'intcpt.p' = 'p.value') %>% 
  select(-term)


fmt_tab_term <- tibble(cluster = c('0', '1', '2', '3', '4', '5', '6', '7'), 
                       Anat = "") %>% 
  dplyr::inner_join(ca) %>% 
  dplyr::inner_join(sex) %>% 
  dplyr::inner_join(pma) %>% 
  dplyr::inner_join(mot_fd) %>% 
  dplyr::inner_join(intcpt) %>% 
  dplyr::mutate(mot_fd.stat = format(round(mot_fd.stat, 
                                           digits = 3), 
                                     nsmall = 3), 
                ca.stat = format(round(ca.stat, 
                                       digits = 3), 
                                 nsmall = 3), 
                sex.stat = format(round(sex.stat, 
                                        digits = 3), 
                                  nsmall = 3), 
                pma.stat = format(round(pma.stat, 
                                        digits = 3), 
                                  nsmall = 3), 
                intcpt.stat = format(round(intcpt.stat, 
                                           digits = 3), 
                                     nsmall = 3), 
                mot_fd.p = format(round(mot_fd.p, 
                                        digits = 3), 
                                  nsmall = 3), 
                ca.p = format(round(ca.p, 
                                    digits = 3), 
                              nsmall = 3), 
                sex.p = format(round(sex.p, 
                                     digits = 3), 
                               nsmall = 3), 
                pma.p = format(round(pma.p, 
                                     digits = 3), 
                               nsmall = 3), 
                intcpt.p = format(round(intcpt.p, 
                                        digits = 3), 
                                  nsmall = 3))

color.mot_fd <- which(as.numeric(fmt_tab_term$mot_fd.p) < 0.05)
color.pma <- which(as.numeric(fmt_tab_term$pma.p) < 0.05)
color.sex <- which(as.numeric(fmt_tab_term$sex.p) < 0.05)
color.ca <- which(as.numeric(fmt_tab_term$ca.p) < 0.05)
color.intcpt <- which(as.numeric(fmt_tab_term$intcpt.p) < 0.05)

fmt_tab_term$mot_fd.p[fmt_tab_term$mot_fd.p == '0.000'] <- '< 0.001'
fmt_tab_term$pma.p[fmt_tab_term$pma.p == '0.000'] <- '< 0.001'
fmt_tab_term$sex.p[fmt_tab_term$sex.p == '0.000'] <- '< 0.001'
fmt_tab_term$ca.p[fmt_tab_term$ca.p == '0.000'] <- '< 0.001'
fmt_tab_term$intcpt.p[fmt_tab_term$intcpt.p == '0.000'] <- '< 0.001'


if (length(color.mot_fd) != 0){
  
  for (i in 1:length(color.mot_fd)){
    
    fmt_tab_term[color.mot_fd[i],9] <- cell_spec(fmt_tab_term[color.mot_fd[i],9], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab_term[color.mot_fd[i],10] <- cell_spec(fmt_tab_term[color.mot_fd[i],10], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    
  }}

if (length(color.pma) != 0){
  
  for (i in 1:length(color.pma)){
    
    fmt_tab_term[color.pma[i],7] <- cell_spec(fmt_tab_term[color.pma[i],7], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab_term[color.pma[i],8] <- cell_spec(fmt_tab_term[color.pma[i],8], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    
  }}

if (length(color.sex) != 0){
  
  for (i in 1:length(color.sex)){
    
    fmt_tab_term[color.sex[i],5] <- cell_spec(fmt_tab_term[color.sex[i],5], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab_term[color.sex[i],6] <- cell_spec(fmt_tab_term[color.sex[i],6], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    
  } 
}

if (length(color.ca) != 0){
  
  for (i in 1:length(color.ca)){
    
    fmt_tab_term[color.ca[i],3] <- cell_spec(fmt_tab_term[color.ca[i],3], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab_term[color.ca[i],4] <- cell_spec(fmt_tab_term[color.ca[i],4], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    
  }}

if (length(color.intcpt) != 0){
  
  for (i in 1:length(color.intcpt)){
    
    fmt_tab_term[color.intcpt[i],11] <- cell_spec(fmt_tab_term[color.intcpt[i],11], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab_term[color.intcpt[i],12] <- cell_spec(fmt_tab_term[color.intcpt[i],12], background = "#4DBBD5FF", color = 'white', bold = TRUE)
...
pt <- pw_ttests %>% 
  ...
  dplyr::mutate(p.value = p.adjust(p.value, 
                                   method = 'fdr', 
                                   n = length(p.value))) %>% 
  dplyr::rename('pt.stat' = 'statistic', 
                'pt.p' = 'p.value') %>% 
  select(-term)
...
fmt_tab <- tibble(cluster = c('0', '1', '2', '3', '4', '5', '6', '7'), 
                  Anat = "") %>% 
  dplyr::inner_join(pt) %>% 
  dplyr::inner_join(sex) %>% 
  dplyr::inner_join(pma) %>% 
...
color.mot_fd <- which(as.numeric(fmt_tab$mot_fd.p) < 0.05)
color.pma <- which(as.numeric(fmt_tab$pma.p) < 0.05)
color.sex <- which(as.numeric(fmt_tab$sex.p) < 0.05)
color.pt <- which(as.numeric(fmt_tab$pt.p) < 0.05)
color.intcpt <- which(as.numeric(fmt_tab$intcpt.p) < 0.05)

fmt_tab$mot_fd.p[fmt_tab$mot_fd.p == '0.000'] <- '< 0.001'
fmt_tab$pma.p[fmt_tab$pma.p == '0.000'] <- '< 0.001'
fmt_tab$sex.p[fmt_tab$sex.p == '0.000'] <- '< 0.001'
fmt_tab$pt.p[fmt_tab$pt.p == '0.000'] <- '< 0.001'
fmt_tab$intcpt.p[fmt_tab$intcpt.p == '0.000'] <- '< 0.001'

if (length(color.mot_fd) != 0){
  
  for (i in 1:length(color.mot_fd)){
    
    fmt_tab[color.mot_fd[i],9] <- cell_spec(fmt_tab[color.mot_fd[i],9], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab[color.mot_fd[i],10] <- cell_spec(fmt_tab[color.mot_fd[i],10], background = "#4DBBD5FF", color = 'white', bold = TRUE)
...
if (length(color.pma) != 0){
  ...
  for (i in 1:length(color.pma)){
    ...
    fmt_tab[color.pma[i],7] <- cell_spec(fmt_tab[color.pma[i],7], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab[color.pma[i],8] <- cell_spec(fmt_tab[color.pma[i],8], background = "#4DBBD5FF", color = 'white', bold = TRUE)
...
if (length(color.sex) != 0){
  ...
  for (i in 1:length(color.sex)){
    ...
    fmt_tab[color.sex[i],5] <- cell_spec(fmt_tab[color.sex[i],5], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab[color.sex[i],6] <- cell_spec(fmt_tab[color.sex[i],6], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    
  } 
}

if (length(color.pt) != 0){
  
  for (i in 1:length(color.pt)){
    
    fmt_tab[color.pt[i],3] <- cell_spec(fmt_tab[color.pt[i],3], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab[color.pt[i],4] <- cell_spec(fmt_tab[color.pt[i],4], background = "#4DBBD5FF", color = 'white', bold = TRUE)
...
if (length(color.intcpt) != 0){
  ...
  for (i in 1:length(color.intcpt)){
    ...
    fmt_tab[color.intcpt[i],11] <- cell_spec(fmt_tab[color.intcpt[i],11], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab[color.intcpt[i],12] <- cell_spec(fmt_tab[color.intcpt[i],12], background = "#4DBBD5FF", color = 'white', bold = TRUE)
...
dt <- dfm %>% 
  ...
  select(-c(pma, pt, sex, motparam_fd_outlier_count)) %>% 
  dplyr::group_by(cluster) %>% 
  dplyr::summarise(pat = list(tibble(sub,mean_dwell))) %>% 
  dplyr::ungroup() %>% 
  tidyr::spread(key = cluster, 
                value = pat) %>% 
  tidyr::unnest() %>% 
...
dt_tab <- tibble::tibble('State' = c('0','1','2','3','4','5','6','7'), 
                         'Anat' = '', 
                         dtx, 
                         'Histogram' = '') %>% 
  dplyr::select(-c(clus, val))
...
dt_list <- split(dtx$val, dtx$clus)
...
if (length(color.ca) != 0){
  ...
  for (i in 1:length(color.ca)){
    ...
    fmt_tab_term[color.ca[i],5] <- cell_spec(fmt_tab_term[color.ca[i],5], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab_term[color.ca[i],6] <- cell_spec(fmt_tab_term[color.ca[i],6], background = "#4DBBD5FF", color = 'white', bold = TRUE)
...
if (length(color.sex) != 0){
  ...
  for (i in 1:length(color.sex)){
    ...
    fmt_tab_term[color.sex[i],3] <- cell_spec(fmt_tab_term[color.sex[i],3], background = "#4DBBD5FF", color = 'white', bold = TRUE)
    fmt_tab_term[color.sex[i],4] <- cell_spec(fmt_tab_term[color.sex[i],4], background = "#4DBBD5FF", color = 'white', bold = TRUE)
